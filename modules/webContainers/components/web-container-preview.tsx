...
const WebContainerPreview = ({
  ...
  useEffect(() => {
    ...
    async function setUpContainer() {
      ...
      if (!instance || isSetupComplete || isSetupInProgress || hasAttemptedSetup) return
      try {
        setIsSetupInProgress(true)
        setSetupError(null)

        try {
          const packageJSONexist = await instance?.fs.readFile("package.json", "utf8")
          console.log(packageJSONexist, "Package Json Exist")

          if (packageJSONexist) {
            if (terminalRef.current?.writeToTerminal) {
              terminalRef.current.writeToTerminal("Reconcting to existing WebContainer instance...\n")
            }
          ...
          console.log(error, "Also error here")
          // throw new Error("Package.json required for node project.")
        }
        ...
        if (installExitCode !== 0) {
          throw new Error(`Failed to install dependendys Exit code: ${installExitCode}`)
        }
        ...
        instance?.on("server-ready", (port: number, url: string) => {
          if (terminalRef.current?.writeToTerminal) {
            terminalRef.current.writeToTerminal(`Development server is running at ${url}\r\n`);
          }
          console.log(url)
          setPreviewUrl(url)
          setLoadingState((prev) => ({
        ...
        //Handle start process output - strem to terminal
    ...
    setUpContainer()
  }, [instance, templateData, isSetupComplete, isSetupInProgress, hasAttemptedSetup])
