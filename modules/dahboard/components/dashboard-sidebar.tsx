...
import { PlaygroundFetch } from "../type"
...
export function DashboardSidebar({ initialPlaygroundData }: { initialPlaygroundData: PlaygroundFetch[] }) {
    const pathname = usePathname()

    console.log("Initital playground data",initialPlaygroundData)
    const [starredPlaygrounds, setStarredPlaygrounds] = useState(initialPlaygroundData.filter((p) => p.favourit?.isMarked))
                        ...
                        <SidebarMenu>
                            ...
                            {starredPlaygrounds.length === 0 && recentPlaygrounds.length === 0 ? (
                                ...
                                starredPlaygrounds.map((playground) => {
                                    const IconComponent = lucideIconMap[playground.icon ?? "Code2"] || Code2;
                                    console.log(playground._id)
                                    return (
                                        <SidebarMenuItem key={playground._id}>
                                            <SidebarMenuButton
                                                asChild
                                                isActive={pathname === `/playground/${playground._id}`}
