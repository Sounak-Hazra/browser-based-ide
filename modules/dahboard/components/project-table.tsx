...
import type { Favourit, PlaygroundFetch } from "../type"
...
interface ProjectTableProps {
    projects: PlaygroundFetch[]
    onUpdateProject?: (id: string, data: { title: string; description: string }) => Promise<ApiSuccessResponse<null> | ApiErrorResponse>
    onDeleteProject?: (id: string) => Promise<ApiSuccessResponse<boolean> | ApiErrorResponse>
    onDuplicateProject?: (id: string) => Promise<ApiSuccessResponse<PlaygroundFetch> | ApiErrorResponse>
    onMarkasFavorite?: (id: string) => Promise<ApiSuccessResponse<Favourit> | ApiErrorResponse> 
}
...
export default function ProjectTable({
    ...
    const [selectedProject, setSelectedProject] = useState<PlaygroundFetch | null>(null)
    const [editData, setEditData] = useState<EditProjectData>({ title: "", description: "" })
    const [isLoading, setIsLoading] = useState(false)
    const router = useRouter()

    const handleEditClick = (project: PlaygroundFetch) => {
    ...
    const handleDeleteClick = async (project: PlaygroundFetch) => {
    ...
    const handleUpdateProject = async () => {
        ...
        try {
            const res = await onUpdateProject(selectedProject._id, editData);
    ...
    const handleMarkasFavorite = async (id: string): Promise<boolean> => {
        try {
            if(!onMarkasFavorite) return false
            const res = await onMarkasFavorite(id)
    ...
    const handleDeleteProject = async () => {
        ...
        try {
            const res = await onDeleteProject(selectedProject._id);
    ...
    const handleDuplicateProject = async (project: PlaygroundFetch) => {
        if (!onDuplicateProject) return;

        setIsLoading(true);

        try {
            const res = await onDuplicateProject(project._id);

            if (res.success) {
                toast.success(res.message || "Project duplicated successfully");
            } else {
                toast.error(res.message || "Failed to duplicate project");
            }
        } catch (error) {
            toast.error("Unexpected error occurred");
            console.log(error);
    ...
    const copyProjectUrl = (projectId: string) => {
        ...
        toast.success("Project Url coppied to clipboard.")
                                ...
                                <TableCell>
                                    <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-full overflow-hidden">
                                            <Image
                                                src={project.userId.image || "/placeholder.svg"}
                                        ...
                                        <span className="text-sm">{project.userId.name}</span>
                                    </div>
                                </TableCell>
                                <TableCell>
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="w-48">
                                            ...
                                            <DropdownMenuItem onClick={() => copyProjectUrl(project._id)}>
